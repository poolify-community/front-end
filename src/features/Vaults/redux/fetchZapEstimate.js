...
import { zapDepositEstimate, zapWithdrawEstimate } from 'libs/web3';
import { convertAmountToRawNumber } from 'libs/helpers/bignumber';
...
export function reducer(state, action) {
  switch (action.type) {
    ...
    case VAULT_FETCH_ZAP_ESTIMATE_SUCCESS:
      const { pools } = state;
      const poolIndex = pools.findIndex(pool => pool.vaultContractAddress === action.vaultAddress);
      pools[poolIndex] = {
        ...pools[poolIndex],
        ...action.data,
      };
